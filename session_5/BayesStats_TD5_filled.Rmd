...
cat(file = "model_lmm4.txt","
    ...
    # Prior on slopes for fixed effects
    for (s in 1:7){
    theta[s] ~ dnorm(0,0.01)
    }
    ...
    for (i in 1:M){
      for(t in 1:T){
        ...
        lambda[i,t] <- mu + site[i] + year[t] + gamma * yr[t] + theta[1] * elev[i] +  theta[2] * elev[i]^2 +
        theta[3] * forest[i] + theta[4] * date[i,t] + theta[5] * date[i,t]^2 + theta[6] *dur[i,t] + theta[7] * twosurveys[i]
        }
      }
...
* date has likely negative or dome-shaped effect : here the effect is weak and opposite to its logical expectation $\Rightarrow$ probably an artefact, we can neglect it. 
...
* Plot the trends
* Examine different priors, e.g. dunif(0, 1) or similar. 
...
With now the added $$\gamma_i \sim \mathcal{N}(\mu_{\gamma,i},\sigma_\gamma^2)$$ 
    ...
    for (i in 1:M){
      for(t in 1:T){
        ...
        lambda[i,t] <- mu + site[i] + year[t] + gamma[i] * yr[t] +
          ...
          theta[3] * forest[i] + theta[4] * dur[i,t] 
        }
    }
...
We found logical and expected result, with positive trends for birds higher up and similar effects of elevation on abundance to earlier models.  
